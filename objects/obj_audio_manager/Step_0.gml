       ...
       if(instance_exists(CharacterCheck)){
          ...
          if(CharacterCheck.CharacterState = CharacterNormal && CharacterCheck.Invincibility = 1.5){
             if(Invincibility_Normal != noone){
                if(CheckSound(bgm_invincibility) == false){
                      StopSound(ZoneBGM);
                   LoopSound(bgm_invincibility, global.BGMVolume, 1);
                   }
                
                }
    
          }else{
                if(CheckSound(bgm_invincibility) == true){
                   StopSound(bgm_invincibility);
       ...
       if(instance_exists(CharacterCheck)){
             ...
             if(CharacterCheck.CharacterState = CharacterSuper or CharacterCheck.CharacterState = CharacterHyper){
                if(Invincibility_Transform != noone){
                   ...
                   if(CheckSound(bgm_super_sonic) == false){
                         StopSound(ZoneBGM)
                      LoopSound(bgm_super_sonic, global.BGMVolume, 1);
                   }
                }
             }else{
                   if(CheckSound(bgm_super_sonic) == true){
                      StopSound(bgm_super_sonic)
                         LoopSound(ZoneBGM, global.BGMVolume, 1);
                   }               
             }
