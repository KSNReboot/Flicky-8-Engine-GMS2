/// @description Apply Animations and Direction.
   ...
   // Walking/Jogging Animation:
      if(Animation == "WALK" && Ground && Action != ActionSlipping){
        if(CharacterID == CharacterSonic){
           AnimationSpeed = min(0.30, max(AnimationSpeed,0.05+abs(XSpeed)/25));
        }
        if(CharacterID == CharacterTails){
           AnimationSpeed = min(0.25, max(AnimationSpeed,0.10+abs(XSpeed)/25));
           TailSpeed      = min(0.20, max(TailSpeed,0.10+abs(XSpeed)/25));
      }
        if(CharacterID == CharacterKnuckles){
           AnimationSpeed = min(0.25, max(AnimationSpeed,0.05+abs(XSpeed)/25));
        }      
        if(CharacterID == CharacterAmy){
           AnimationSpeed = min(0.20, max(AnimationSpeed,0.05+abs(XSpeed)/25));
        } 
   ...
   // Amy's Hammer Spin Animation:  
      if(CharacterID == CharacterAmy){
         if(AmyHammerAttack == 2){
            AnimationSpeed = min(0.30, max(AnimationSpeed,0.10+abs(XSpeed)/25));
         }
      }
...
draw_ellipse_color(x-size/2-__view_get( e__VW.XView, 0 ),y-size/2-__view_get( e__VW.YView, 0 ),x+size/2-__view_get( e__VW.XView, 0 ),y+size/2-__view_get( e__VW.YView, 0 ),c_white,c_black,false)
