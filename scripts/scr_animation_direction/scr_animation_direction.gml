...
function scr_animation_direction() {
	// To correctly change the direction!
	  ...
	  // Change the direction in the air:
	     if(!Ground && Action = ActionNormal){
	         ...
	         // If the right key is held, change the direction:
	            if(scr_character_collision_bottom_object(x, y, 0, spr_mask_big, par_collision_slipping_right)) { AnimationDirection =  1 }
         
	         // If the left key is held, change the direction:
	            if(scr_character_collision_bottom_object(x, y, 0, spr_mask_big, par_collision_slipping_left)) { AnimationDirection = -1 }      
             
	 if Action = ActionBarV || Action = ActionBarH{
