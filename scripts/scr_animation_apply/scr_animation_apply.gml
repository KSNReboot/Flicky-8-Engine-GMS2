...
function scr_animation_apply() {
	// Checks the current state of the character and applies the correct animation:
	  ...
	  // Frame counter, simply count frames when an animation is being played. Really useful for a lot of things!
	     ...
	     if(CharacterID == CharacterSonic || CharacterID = CharacterAmy || CharacterID = CharacterMighty){
	        switch(Animation){
	  ...
	  if((CharacterID == CharacterSonic && CharacterState = CharacterNormal) || CharacterID == CharacterTails || CharacterID == CharacterMighty || CharacterID == CharacterRay){
	     ...
	     // When the character is in his normal state:
	        if(Action = ActionNormal && Ground){
	           ...
	           // Run Animation:
	              if(XSpeed != 0 && PeeloutFlag == 0 && Animation != "RUN" && abs(XSpeed) >= 5.50 && Action != ActionSlipping){
	                 Animation = "RUN";
	              }  
	  ...
	  if(CharacterID == CharacterSonic && (CharacterState = CharacterSuper or CharacterState = CharacterHyper)){
	     ...
	     // When the character is in his normal state:
	        if(Action = ActionNormal && Ground){
	           ...
	           // Run Animation:
	              if(XSpeed != 0 && PeeloutFlag == 0 && Animation != "RUN" && abs(XSpeed) >= 6  && Action != ActionSlipping){
	                 Animation = "RUN";
	              }  
	  ...
	  if((CharacterID == CharacterKnuckles)){
	     ...
	     // When the character is in his normal state:
	        if(Action = ActionNormal && Ground){
	           ...
	           // Jog Animation:
	              if(abs(XSpeed) != 0 && abs(XSpeed) < 5.50 && Animation != "WALK" && Action != ActionSlipping){
	                 Animation = "WALK";
	              }
